...
public class UserEditController extends Controller {
	...
	public UserEditController(ControllerInterface controller, String document_keyword) { super(controller, document_keyword); }
	...
	public void createControls(Template document, Object model) throws Exception {
		...
		if(!getIsPostback()) {
			String sGuid = getRequest().getParameter(User.GUID);
			if(sGuid != null) {
				objModel = User.loadByGuid(getUser().login(), User.class, sGuid);
			} else {
				objModel = new User();
				objModel.initialize();
			}
	...
	private void btnSave_OnClick() throws Exception {
		...
		redirect("~/incBootstrap?App=UserEdit&GUID=" + objModel.getGuid() + "&Error=Saved!");
	}
