...
public class SalesTaxEditController extends Controller {
	...
	public void createControls(Template document, Object model) throws Exception { 
		...
		if(!getIsPostback()) {
			...
			if(sGuid != null)
				objModel = SalesTax.loadByGuid(getUser().login(), SalesTax.class, sGuid);
			else {
				objModel = new SalesTax();
				objModel.initialize();
			}
		}
		setModel(objModel);
	...
	private SalesTaxEditOptionController createController(SalesTaxMembershipOption option) {
		SalesTaxEditOptionController steoc = new SalesTaxEditOptionController(this, "Option");
	...
	public void btnSave_OnClick() throws Exception {
		...
		redirect("~/incAccounting?App=SalesTaxEdit&GUID=" + objModel.getGuid() + "&Error=Saved!");
	}
