...
public class ItemEditController extends Controller {
	public ItemEditController(ControllerInterface controller, String document_keyword) { super(controller, document_keyword); }
	public boolean getEnforceSecurity() { return true; }
	public void createControls(Template document, Object model) throws Exception {
		setDocument(new Template(read_url("~/templates/ItemEdit.html"), new HtmlSyntax()));

		Item objModel = (Item) model;
		if(!getIsPostback()) {
			String sGuid = getRequest().getParameter("GUID");
			if(sGuid != null)
				objModel = Item.loadByGuid(getUser().login(), Item.class, sGuid);
			else {
				objModel = new Item();
				objModel.initialize();
...
//		txtInventoryDescription.setRows(getUser().getSetting(Item.SETTING_DESCRIPTION_ROWS));
	...
	private ItemEditChildController createController(Item item) {
		Item objModel = (Item) getModel();

		ItemEditChildController iecc = new ItemEditChildController(this, "Child");
	...
	private void btnSave_OnClick() throws Exception {
		...
		redirect("~/incAccounting?App=ItemEdit&GUID=" + objModel.getGuid() + "&Error=Saved!");
	}
	private void btnOpen_OnClick() throws Exception {
		Item objModel = (Item) getModel();
		
		if(objModel.getParentItemsGuid() != null)
			redirect("~/incAccounting?App=ItemEdit&GUID=" + objModel.getParentItemsGuid());		
	}
	private void btnAdd_OnClick() throws Exception {
		Item objModel = (Item) getModel();
		redirect("~/incAccounting?App=ItemEdit&" + Item.PARENT_ITEMS_GUID + "=" + objModel.getGuid());	
	}
