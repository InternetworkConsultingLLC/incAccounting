...
public class PayrollCheckEditController extends Controller {
	private PayrollCheck objModel;
	private ButtonTag btnTemplate;
	public PayrollCheckEditController(ControllerInterface controller, String document_keyword) { super(controller, document_keyword); }
	...
	private PayrollCheckEditFieldController createFieldController(PayrollFieldValue objField, String sBlock, String sFieldTypeLimit) throws Exception {
		if(sFieldTypeLimit == null)
			throw new Exception("Field type limit not specified!");

		PayrollCheckEditFieldController pcefc = new PayrollCheckEditFieldController(this, sBlock);
	...
	public void beforePopulate() throws Exception {
		...
		if(!objModel.loadCompanyPaidValues(getUser().login(), false).isEmpty())
			btnTemplate.setIsReadOnly(true);
	...
	private void btnAddCompensation_OnClick() throws Exception {
		...
		objModel.loadGrossExepnseValues(getUser().login(), false).add(pfv);
		
		PayrollCheckEditFieldController pcefc = createFieldController(pfv, "Gross", PayrollFieldType.TYPE_GROSS_EXPENSE_GUID);		
	...
	private void btnAddWithholding_OnClick() throws Exception {
		...
		objModel.loadEmployeePaidValues(getUser().login(), false).add(pfv);

		PayrollCheckEditFieldController pcefc = createFieldController(pfv, "Employee", PayrollFieldType.TYPE_EMPLOYEE_PAID_GUID);		
		doCreateControls(pcefc, false);
		doBeforeUpdate(pcefc);
		doUpdateControls(pcefc);
		doBeforeHandle(pcefc);
	...
	private void btnAddContribution_OnClick() throws Exception {
		...
		objModel.loadCompanyPaidValues(getUser().login(), false).add(pfv);

		PayrollCheckEditFieldController pcefc = createFieldController(pfv, "Company", PayrollFieldType.TYPE_COMPANY_PAID_GUID);		
		doCreateControls(pcefc, false);
		//doBeforeUpdate(pcefc);
		//doUpdateControls(pcefc);
		//doBeforeHandle(pcefc);
	...
	private void btnSave_OnClick() throws Exception {
		...
		redirect("~/incAccounting?App=PayrollCheckEdit&GUID=" + objModel.getGuid());
	}
	private void btnPost_OnClick() throws Exception {
		...
		redirect("~/incAccounting?App=PayrollCheckEdit&GUID=" + objModel.getGuid() + "&Error=" + sAction + "!");
	}
